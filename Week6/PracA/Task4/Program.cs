...
class Progress
{
    ...
    public static Queue<BankAccount> SortQueue(Queue<BankAccount> accountQueue)
{
    Queue<BankAccount> sortedQueue = new Queue<BankAccount>();

    int count = accountQueue.Count;

    for (int i = 0; i < count; i++)
    {
        Queue<BankAccount> tempQueue = new Queue<BankAccount>();

        BankAccount lowestAccount = accountQueue.Peek();
        decimal lowestBalance = lowestAccount.Balance;

        while (accountQueue.Count > 0)
        {
            BankAccount currentAccount = accountQueue.Dequeue();

            if (currentAccount.Balance < lowestBalance)
            {
                lowestBalance = currentAccount.Balance;
                lowestAccount = currentAccount;
            }

            tempQueue.Enqueue(currentAccount);
        }

        while (tempQueue.Count > 0)
        {
            BankAccount currentAccount = tempQueue.Dequeue();

            if (currentAccount != lowestAccount)
            {
                accountQueue.Enqueue(currentAccount);
            }
        }

        sortedQueue.Enqueue(lowestAccount);
    }
    System.Console.WriteLine();
    System.Console.WriteLine();
    System.Console.WriteLine("After sort");
    foreach(BankAccount account5 in sortedQueue)
        {
            System.Console.WriteLine(account5);
        }

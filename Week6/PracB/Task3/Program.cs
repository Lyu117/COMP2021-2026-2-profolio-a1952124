...
class Progress
{
    public static void Main()
    {
        ...
        account.Push(a10);
         ...
         var stackResult = account.Where(x => x.Owner.Contains("e"))
         .Select(x => new { x.Owner, x.Balance }).OrderBy(x => x.Balance);
         System.Console.WriteLine("Stack Owner");
         foreach(var a in stackResult)
        {
        ...
        var queueResult = accountQueue.Where(x => x.Owner.Contains("e"))
        .Select(x => new { x.Owner, x.Balance }).OrderBy(x => x.Balance);
        System.Console.WriteLine("Queue Owner");
        foreach(var b in queueResult)
        {
